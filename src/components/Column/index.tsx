...
const Column: FC<Props> = ({ column, columnIndex, defaultIsShowRows = false }) => {
	...
	const handleCanDrop = (dragItem: ColumnDragItem, dropZone: ColumnDropZone): boolean =>
		dragItem?.columnIndex !== dropZone?.columnIndex;

	return (
		<div className={`column ${isDragging ? 'column_dragging' : ''}`} ref={drag} draggable>
			<DropZone
				accept='column'
				dropZoneData={{ ...column, columnIndex }}
				onDrop={handleDrop}
					...
					{isShowRows && (
						<div>
							{column?.rows?.length > 0 ? (
								column?.rows?.map((row: IRow, index: number) => (
